...
import type { WorkItemRecord, WorkProjectRecord } from "./work/model.js";
...
export class Session {
  ...
  private unsubscribeWorkMutations: (() => void) | null = null;
  ...
  private subscribeToWorkMutations(): void {
    if (this.unsubscribeWorkMutations) {
      this.unsubscribeWorkMutations();
      this.unsubscribeWorkMutations = null;
    }
    if (!this.workService) return;
    const sub = this.workService.subscribeToMutations.bind(this.workService);
    if (!sub) return;
    this.unsubscribeWorkMutations = sub(async (mutation: unknown) => {
      const m = mutation as { entity?: string; kind?: string; id?: string; record?: unknown };
      // Gate on workBoard feature the same way neighbouring pushes gate: server always emits, client gates on feature flag.
      // No per-session workBoard gate here; the capability is advertised via server_info.features.workBoard.
      try {
        if (m.entity === "item") {
          if (m.kind === "remove") {
            const id = String(m.id ?? "");
            const projectKey = (m.record as { projectKey?: string } | null)?.projectKey ?? "";
            // For removes the record is null, so we need projectKey from removed item's stored identity if available.
            // Fallback to empty projectKey - client can still drop by id.
            this.emit({
              type: "work.item.updated",
              payload: { kind: "remove", itemId: id, projectKey: projectKey || "unknown" },
            });
            return;
          }
          // upsert/reorder -> push full wire item
          const record = m.record as WorkItemRecord | null;
          if (!record) return;
          const wire = await this.workService!.getItem(record.id)
            .then((r) => r.detail?.item ?? null)
            .catch(() => null);
          if (wire) {
            this.emit({ type: "work.item.updated", payload: { kind: "upsert", item: wire } });
          }
        } else if (m.entity === "project") {
          if (m.kind === "remove") {
            this.emit({
              type: "work.project.updated",
              payload: { kind: "remove", projectKey: String(m.id ?? "") },
            });
            return;
          }
          const record = m.record as WorkProjectRecord | null;
          if (!record) return;
          this.emit({
            type: "work.project.updated",
            payload: {
        ...
        this.sessionLogger.warn({ err: error, mutation: m }, "Failed to emit work update");
      }
    });
  }
   ...
   * the agent has not been stopped yet (additive).
   */
  ...
  > = [
    ...
    (msg) => this.dispatchWorkMessage(msg),
  ...
  private dispatchWorkMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    return (
      this.dispatchWorkItemMessages(msg) ??
      this.dispatchWorkCommentActivityMessages(msg) ??
      this.dispatchWorkLabelMessages(msg) ??
      this.dispatchWorkPageMessages(msg) ??
      this.dispatchWorkDraftStickyViewMessages(msg)
    );
  }

  private dispatchWorkItemMessages(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      case "work.project.list.request":
        return this.handleWorkProjectListRequest(msg);
      case "work.item.list.request":
        return this.handleWorkItemListRequest(msg);
      case "work.item.get.request":
        return this.handleWorkItemGetRequest(msg);
      case "work.item.create.request":
        return this.handleWorkItemCreateRequest(msg);
      case "work.item.update.request":
        return this.handleWorkItemUpdateRequest(msg);
      case "work.item.delete.request":
        return this.handleWorkItemDeleteRequest(msg);
      case "work.item.move.request":
        return this.handleWorkItemMoveRequest(msg);
      case "work.item.dispatch.request":
        return this.handleWorkItemDispatchRequest(msg);
      default:
        return undefined;
    }
  }

  private dispatchWorkCommentActivityMessages(
    msg: SessionInboundMessage,
  ): Promise<void> | undefined {
    switch (msg.type) {
      case "work.comment.list.request":
        return this.handleWorkCommentListRequest(msg);
      case "work.comment.create.request":
        return this.handleWorkCommentCreateRequest(msg);
      case "work.activity.list.request":
        return this.handleWorkActivityListRequest(msg);
      default:
        return undefined;
    }
  }

  private dispatchWorkLabelMessages(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      case "work.label.list.request":
        return this.handleWorkLabelListRequest(msg);
      case "work.label.upsert.request":
        return this.handleWorkLabelUpsertRequest(msg);
      case "work.label.delete.request":
        return this.handleWorkLabelDeleteRequest(msg);
      default:
        return undefined;
    }
  }

  private dispatchWorkPageMessages(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      case "work.page.list.request":
        return this.handleWorkPageListRequest(msg);
      case "work.page.get.request":
        return this.handleWorkPageGetRequest(msg);
      case "work.page.upsert.request":
        return this.handleWorkPageUpsertRequest(msg);
      case "work.page.delete.request":
        return this.handleWorkPageDeleteRequest(msg);
      default:
        return undefined;
    }
  }

  private dispatchWorkDraftStickyViewMessages(
    msg: SessionInboundMessage,
  ): Promise<void> | undefined {
    switch (msg.type) {
      case "work.draft.list.request":
        return this.handleWorkDraftListRequest(msg);
      case "work.draft.create.request":
        return this.handleWorkDraftCreateRequest(msg);
      case "work.draft.promote.request":
        return this.handleWorkDraftPromoteRequest(msg);
      case "work.sticky.list.request":
        return this.handleWorkStickyListRequest(msg);
      case "work.sticky.upsert.request":
        return this.handleWorkStickyUpsertRequest(msg);
      case "work.sticky.delete.request":
        return this.handleWorkStickyDeleteRequest(msg);
      case "work.view.list.request":
        return this.handleWorkViewListRequest(msg);
      case "work.view.upsert.request":
        return this.handleWorkViewUpsertRequest(msg);
      default:
        return undefined;
    }
  }
  ...
  private async handleWorkItemListRequest(
    ...
    this.emit({
      ...
      payload: { requestId: msg.requestId, projectKey: result.projectKey, hosts: result.hosts },
    });
  }

  private async handleWorkItemGetRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.get.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService ? await this.workService.getItem(msg.id) : { detail: null };
      this.emit({
        type: "work.item.get.response",
        payload: { requestId: msg.requestId, detail: result.detail },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
      this.emit({
        type: "rpc_error",
        payload: {
          requestId: msg.requestId,
  ...
  private async handleWorkItemCreateRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.create.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.createItem({
      ...
      : { item: null, error: "Work not available" };
    this.emit({
      type: "work.item.create.response",
      payload: { requestId: msg.requestId, item: result.item, error: result.error ?? null },
    });
  }

  private async handleWorkItemUpdateRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.update.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.updateItem(msg.id, {
            title: msg.title,
            description: msg.description,
            priority: msg.priority,
            labelIds: msg.labelIds,
        ...
        : { item: null, error: "Work not available" };
      this.emit({
        type: "work.item.update.response",
        payload: { requestId: msg.requestId, item: result.item, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkItemDeleteRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.delete.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.deleteItem(msg.id)
        : { success: false, error: "Work not available" };
      this.emit({
        type: "work.item.delete.response",
        payload: { requestId: msg.requestId, success: result.success, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkItemMoveRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.move.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.moveItem({
            id: msg.id,
      ...
      if (result.error) {
        this.emit({
          type: "work.item.move.response",
          payload: { requestId: msg.requestId, item: null, error: result.error },
        });
        return;
      }
      this.emit({
        ...
        payload: {
          requestId: msg.requestId,
          item: result.item,
          ...(result.rebalanced ? { rebalanced: result.rebalanced } : {}),
          error: null,
        },
      });
    } catch (error) {
  ...
  private async handleWorkItemDispatchRequest(
    msg: Extract<SessionInboundMessage, { type: "work.item.dispatch.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.dispatchItem(msg.id)
        : { item: null, error: "Work not available" };
      this.emit({
        type: "work.item.dispatch.response",
        payload: { requestId: msg.requestId, item: result.item, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkCommentListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.comment.list.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.listComments(msg.itemId)
        : { itemId: msg.itemId, comments: [] };
      this.emit({
        type: "work.comment.list.response",
        payload: { requestId: msg.requestId, itemId: result.itemId, comments: result.comments },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkCommentCreateRequest(
    msg: Extract<SessionInboundMessage, { type: "work.comment.create.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.createComment(msg.itemId, msg.body)
        : { comment: null, error: "Work not available" };
      this.emit({
        type: "work.comment.create.response",
        payload: { requestId: msg.requestId, comment: result.comment, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkActivityListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.activity.list.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.listActivity(msg.itemId)
        : { itemId: msg.itemId, activity: [] };
      this.emit({
        type: "work.activity.list.response",
        payload: { requestId: msg.requestId, itemId: result.itemId, activity: result.activity },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkLabelListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.label.list.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.listLabels(msg.projectKey)
      : { projectKey: msg.projectKey, labels: [] };
    this.emit({
      type: "work.label.list.response",
      payload: { requestId: msg.requestId, projectKey: result.projectKey, labels: result.labels },
    });
  }

  private async handleWorkLabelUpsertRequest(
    msg: Extract<SessionInboundMessage, { type: "work.label.upsert.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.upsertLabel(msg.projectKey, msg.label)
        : { label: null, error: "Work not available" };
      this.emit({
        type: "work.label.upsert.response",
        payload: { requestId: msg.requestId, label: result.label, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkLabelDeleteRequest(
    msg: Extract<SessionInboundMessage, { type: "work.label.delete.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.deleteLabel(msg.id)
        : { success: false, error: "Work not available" };
      this.emit({
        type: "work.label.delete.response",
        payload: { requestId: msg.requestId, success: result.success, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkPageListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.page.list.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.listPages(msg.projectKey)
      : { projectKey: msg.projectKey, pages: [] };
    this.emit({
      type: "work.page.list.response",
      payload: { requestId: msg.requestId, projectKey: result.projectKey, pages: result.pages },
    });
  }

  private async handleWorkPageGetRequest(
    msg: Extract<SessionInboundMessage, { type: "work.page.get.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService ? await this.workService.getPage(msg.id) : { page: null };
      this.emit({
        type: "work.page.get.response",
        payload: { requestId: msg.requestId, page: result.page },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkPageUpsertRequest(
    msg: Extract<SessionInboundMessage, { type: "work.page.upsert.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.upsertPage(msg.projectKey, msg.page)
        : { page: null, error: "Work not available" };
      this.emit({
        type: "work.page.upsert.response",
        payload: { requestId: msg.requestId, page: result.page, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkPageDeleteRequest(
    msg: Extract<SessionInboundMessage, { type: "work.page.delete.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.deletePage(msg.id)
        : { success: false, error: "Work not available" };
      this.emit({
        type: "work.page.delete.response",
        payload: { requestId: msg.requestId, success: result.success, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkDraftListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.draft.list.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.listDrafts(msg.projectKey)
      : { projectKey: msg.projectKey, drafts: [] };
    this.emit({
      type: "work.draft.list.response",
      payload: { requestId: msg.requestId, projectKey: result.projectKey, drafts: result.drafts },
    });
  }

  private async handleWorkDraftCreateRequest(
    msg: Extract<SessionInboundMessage, { type: "work.draft.create.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.createDraft({
          projectKey: msg.projectKey,
          title: msg.title,
          description: msg.description,
          priority: msg.priority,
      ...
      : { draft: null, error: "Work not available" };
    this.emit({
      type: "work.draft.create.response",
      payload: { requestId: msg.requestId, draft: result.draft, error: result.error ?? null },
    });
  }

  private async handleWorkDraftPromoteRequest(
    msg: Extract<SessionInboundMessage, { type: "work.draft.promote.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.promoteDraft(msg.id)
        : { item: null, error: "Work not available" };
      this.emit({
        type: "work.draft.promote.response",
        payload: { requestId: msg.requestId, item: result.item, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkStickyListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.sticky.list.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.listStickies(msg.projectKey)
      : { projectKey: msg.projectKey, stickies: [] };
    this.emit({
      type: "work.sticky.list.response",
      payload: {
        requestId: msg.requestId,
  ...
  private async handleWorkStickyUpsertRequest(
    msg: Extract<SessionInboundMessage, { type: "work.sticky.upsert.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.upsertSticky(msg.projectKey, msg.sticky)
        : { sticky: null, error: "Work not available" };
      this.emit({
        type: "work.sticky.upsert.response",
        payload: { requestId: msg.requestId, sticky: result.sticky, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
  ...
  private async handleWorkStickyDeleteRequest(
    msg: Extract<SessionInboundMessage, { type: "work.sticky.delete.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.deleteSticky(msg.id)
        : { success: false, error: "Work not available" };
      this.emit({
        type: "work.sticky.delete.response",
        payload: { requestId: msg.requestId, success: result.success, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async handleWorkViewListRequest(
    msg: Extract<SessionInboundMessage, { type: "work.view.list.request" }>,
  ): Promise<void> {
    const result = this.workService
      ? await this.workService.listViews(msg.projectKey)
      : { projectKey: msg.projectKey, views: [] };
    this.emit({
      type: "work.view.list.response",
      payload: { requestId: msg.requestId, projectKey: result.projectKey, views: result.views },
    });
  }

  private async handleWorkViewUpsertRequest(
    msg: Extract<SessionInboundMessage, { type: "work.view.upsert.request" }>,
  ): Promise<void> {
    try {
      const result = this.workService
        ? await this.workService.upsertView(msg.projectKey, msg.view)
        : { view: null, error: "Work not available" };
      this.emit({
        type: "work.view.upsert.response",
        payload: { requestId: msg.requestId, view: result.view, error: result.error ?? null },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const code = message.includes("unreachable") ? "peer_unreachable" : undefined;
