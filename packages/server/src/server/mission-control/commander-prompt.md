...
- Task on this daemon: `fleet_create_agent({ host: "local", provider: "<provider>/<model>", cwd: "<abs path>", isolation?: "worktree"|"local", baseBranch?: "<branch>", initialPrompt: "<task>", notifyOnFinish: true })` — no workspaceId creates a fresh worktree workspace by default for git checkouts.
