...
1. **Local Mac** — auto-commits any uncommitted changes (commit message written by the `claude` CLI on **Haiku 4.5**, falling back to a timestamp), fetches `upstream`, fast-forwards `origin/main` to `upstream/main`, **merges** `upstream/main` into the custom branch (resolving any conflicts in a single pass with the `grok` CLI on **Grok 4.5 at `high` effort** — streaming its output — then creating one merge commit), pushes to `origin`, runs `npm run build:server` with the Node version from `.tool-versions` (via nvm), and restarts the production-style daemon at `~/.paseo`. Finally it builds the **desktop app** (unsigned) and installs it as `/Applications/Paseo Test.app` — skip with `PASEO_BUILD_DESKTOP=0`, retarget with `PASEO_DESKTOP_TEST_APP`. Models are overridable via `PASEO_COMMIT_MSG_MODEL` / `PASEO_CONFLICT_MODEL` / `PASEO_CONFLICT_EFFORT`. A merge commit is used deliberately (simpler + one-pass resolution); linear history is not preserved.
