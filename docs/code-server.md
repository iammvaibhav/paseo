...
- Clicking a changed file in **Changes** opens VS Code Web's diff for it: the pane context's `openDiffInBrowserEditor` (built in `workspace-screen.tsx`, Electron-only, needs a host **VS Code Web URL**) is consumed by `ChangesPanel` (`panels/diff-panel.tsx`), which re-anchors submodule-relative paths before forwarding to `ChangesSurface`'s `onOpenDiff`, carrying the pane's current base ref. The pane's inline diff and the Changes tab only take that click back where the host has no VS Code Web URL — on mobile, or before one is configured. **Open file** in the kebab is the way to the plain file.
- The diff does not use the git extension. `git:` URIs only resolve for a repository that extension has opened, and it never opens submodules here — `git.detectSubmodules` is off, and these superprojects exceed `git.detectSubmodulesLimit` anyway — so every SCM-backed command silently fails for exactly the files Paseo lists. The bridge serves the read-only side itself from `git show` under its own `paseo-git:` scheme (`registerFileSystemProvider` in `extension.js`), which needs no repository model. Uncommitted view: `HEAD` against the working file, so the right side stays editable. Committed view: `baseRef ↔ HEAD`, matching what the pane lists.
- `diffEditor.hideUnchangedRegions.enabled` (in `user-settings.json` and this Mac's live settings) makes those diffs open collapsed to the changed hunks instead of the whole file.
- The bridge reports a diff open as soon as the **tab exists** (`hasDiffTab`), never by awaiting `vscode.diff`. That command settles only once the diff has been _computed_, and VS Code caps its own algorithm at `diffEditor.maxComputationTime` (5s) — so a heavily rewritten file blew past any worker timeout that fits the broker's 4s request budget, and the app then reloaded away from a diff editor that was already on screen, landing on the plain file. Expect VS Code's own "diff algorithm was stopped early" notice on such files; that is its compute cap, not a failure.
