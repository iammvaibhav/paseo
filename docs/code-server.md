...
- Clicking a changed file in **Changes** opens VS Code Web's diff for it (`GitDiffPane` binds the pane's base ref into the row's `onFilePress`). The pane's inline diff and the Changes tab only take that click back where the host has no VS Code Web URL — on mobile, or before one is configured. **Open file** in the kebab is the way to the plain file.
