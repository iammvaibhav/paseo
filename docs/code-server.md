...
- A changed-file row in **Changes** carries a one-click diff icon plus **Open file** / **Open diff** in its kebab. Both are absent unless the host has VS Code Web, since there is no in-app diff editor to route them to — the pane's own inline diff already covers that case.
- **Open diff** does not use the git extension. `git:` URIs only resolve for a repository the git extension has opened, and a superproject over `git.detectSubmodulesLimit` (10) never opens its submodules — so every SCM-backed command silently fails for exactly the files Paseo lists. The bridge serves the read-only side itself from `git show` under its own `paseo-git:` scheme (`registerFileSystemProvider` in `extension.js`), which needs no repository model and works in a submodule. Uncommitted view: `HEAD` against the working file, so the right side stays editable. Committed view: `baseRef ↔ HEAD`, matching what the pane lists — the pane owns the base ref and binds it, because the row only knows its path.
