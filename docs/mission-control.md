...
Work reuses the same bucket. A Work item has no `status` field; its board column is derived from its lane and the linked agent's `bucket` by `deriveWorkColumn` (`packages/protocol/src/work/state.ts`), and dragging a card to In Review or Done writes the same `reviewState` that feeds `deriveLifecycleBucket`. See [work.md](work.md) for the full state rule and drag table.
