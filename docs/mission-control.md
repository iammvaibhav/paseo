...
The board and feed never wait on any LLM. Events hit the store and push to clients instantly. The digest queue only gates when the Commander's model reads them. The digest→Commander path is being replaced by per-turn world snapshots (commander.md, roadmap M3).
