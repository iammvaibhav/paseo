...
- **THE ask-mode rule (user decision, verbatim: "apart from nudge, everything should require my approval in ask mode. Spinning up a new agent as well, everything.")** — in Ask mode EVERY action class waits for Approve/Edit/Deny EXCEPT the status-ask nudge. The exemption is a single explicit predicate, `isAskModeAutoSendExempt(forceSend, allowPairActive)` in `approvals.ts`, pinned by an enumerated test: status-ask nudge (forceSend, auto-sent) and a user-granted verifier allow-pair are the ONLY auto-sends; everything else — escalation/recovery interrupts, Commander `fleet_create_agent` spawns, verifier spawns, verifier→worker contacts, worker→verifier replies, Commander→worker sends (`fleet_send_prompt`) — is pending in Ask mode. Spawn proposals show what would be spawned (host, provider/model, brief summary) so approving is informed.
...
Every background mechanism logs structured lines under `module: "mission-control"` with `component`: `digest` (queue depth, flush, ack-drop), `verifier` (spawn, exchange, verdict), `approvals` (proposal created/resolved/sent/expired/undelivered), `stall` (nudge/escalate/watchdog-heal), `dormant-turn` (hard-stop recovery), `steer-verify` (delivery verification), `turn-lifecycle` (turn-step transitions with ages), `context` (snapshot injected, size), `naming` (re-map, backfill). `grep mission-control ~/.paseo/daemon.log` must tell the whole story.
